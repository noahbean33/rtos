...
#define INTERRUPT_CONTROL_STATE_REGISTER   (*((volatile uint32_t *) 0xE000ED04))
#define PENDSVSET  (1UL<<26)
...
{
	/* Clear the content of the SysTick Current Value Register */
	SysTick->VAL = 0;

	/* ****************************************************************************************************************************
	 * PENDSTSET RW SysTick exception set-pending bit.
		Write:
		0 = no effect
	...
	/* Set the Interrupt Control and State Register ICSR registry */
	INTERRUPT_CONTROL_STATE_REGISTER = PENDSVSET;
}
...
void tim2_1hz_interrupt_init(void)
{
	...
	/* Set the prescaler */
    TIM2->PSC = 1600 - 1;   // 16 000 000 / 1600 = 10 000  => autoreload value is 10000

	/* Set auto-reload value */

    // To determine the ARR (Auto-Reload) value for a timer, you need to consider the desired time period or frequency you want to achieve with the timer.
    /* *********************************************************************************************************************************************************************
     *Frequency Calculation:
		If you want to determine the ARR value for a specific frequency, the formula becomes:
		ARR = (Timer Clock Frequency / Prescaler) / Frequency - 1
		Again, "Timer Clock Frequency" is the frequency of the timer's input clock, and the "Prescaler" is the division factor applied to the input clock. The result is the desired ARR value, minus 1.
     ********************************************************************************************************************************************************************** */
...
{
	//we wish that all operations to be atomic and not to be interrupted by anything, hence we disable interruptions at this point
	__disable_irq();
	*semaphore +=1;
