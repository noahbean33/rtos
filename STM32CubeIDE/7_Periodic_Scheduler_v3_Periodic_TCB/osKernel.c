...
uint8_t osKernelAddPeriodThreads(void(*thread1)(void),uint32_t period1,void(*thread2)(void),uint32_t period2)
{
	...
	periodicTask[0].task = thread1;
	periodicTask[0].period = period1;
	periodicTask[1].task = thread2;
	periodicTask[1].period = period2;
...
void osSchedulerPeriodicRR(void)
{
	...
	{
		if((TimeMsec % periodicTask[i].period) ==0 && periodicTask[i].task != NULL)
		{
			periodicTask[i].task();
		}
	}
