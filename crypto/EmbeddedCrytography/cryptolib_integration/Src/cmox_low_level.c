...
#define RCC_CRC_RELEASE_RESET()    (RCC->AHB1RSTR &= ~(RCC_AHB1RSTR_CRCRST))




void RCC_CRC_CLK_ENABLE(void)
{
	SET_BIT(RCC->AHB1ENR, RCC_AHB1ENR_CRCEN);
	__IO uint32_t tmpreg =  READ_BIT(RCC->AHB1ENR, RCC_AHB1ENR_CRCEN);
	UNUSED(tmpreg);
}
