...
pub fn configure_syscfg(port: u32, pin: u32) {
    ...
    match port {
        ...
        GPIOG_BASE => {
            reg_set_bits(syscfg_reg_addr, 6, bit_position, 4); // GPIOG is AF 6
        }
        _ => (),
            // Optional: handle or ignore unsupported ports
            // You can log, panic, or just do nothing
...
fn configure_interrupt(exti_line: ExtiLine, is_enable: bool) {
    let exti_imr_addr = (EXTI_BASE + 0x00) as *mut u32; // EXTI_IMR
...
pub fn clear_pending_interrupt(exti_line: ExtiLine) {
    ...
    if line <= 22 {
        ...
        // Optional: handle error for unsupported EXTI lines
        // e.g., panic!("Invalid EXTI line on STM32F429");
    }
}
